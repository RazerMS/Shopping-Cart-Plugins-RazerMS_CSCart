                ...
                $.get("./?dispatch=molpay.getChannel", function(data){ // Preset value from DB
                        ...
                        setTimeout(function(){
                                for(var j=0; j<arrayChannel.length; j++){
                                        $("#payment_method").append("<div class='col-md-2 col-xs-6 marginbttm text-center "+currency+"'><label class='hand' for='payment"+arrayChannel[j]+"'><img src='images/molpay/"+arrayChannel[j]+".jpg' /></label><br /><input style='margin: 8px 0px 0px 50px;' type='radio' name='payment_options' id='payment"+arrayChannel[j]+"' value='"+arrayChannel[j]+"' required/></div>");
                                }
                        }, 3000);
                });
