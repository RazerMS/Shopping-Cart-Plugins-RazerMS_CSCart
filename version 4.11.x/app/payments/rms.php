...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    $molpay_currency = $order_info['secondary_currency'];
    $molpay_channel = $processor_data['processor_params']['channel'];

    //Order Total
    $molpay_shipping = fn_order_shipping_cost($order_info);
    $molpay_total = fn_format_price_by_currency($order_info['total'], CART_PRIMARY_CURRENCY, $order_info['secondary_currency']);
    ...
    $return_url = "http" . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . $_SERVER['HTTP_HOST'] . (substr($_SERVER['REQUEST_URI'], 0, 1) == '/' ? $_SERVER['REQUEST_URI'] : '/' . $_SERVER['REQUEST_URI']) . "?dispatch=payment_notification.return&payment=rms&order_id=" . $orderid;
