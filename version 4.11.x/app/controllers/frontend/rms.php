...
if ($mode == "setup") {
    $data = array(
        "processor" => "Razer Merchant Services Online Payment",
        "processor_script" => "rms.php",
        "processor_template" => "views/orders/components/payments/cc_outside.tpl",
        "admin_template" => "rms.tpl",
        "callback" => "N",
        "type" => "P",
        "addon" => ""
    );
    ...
    if (!$chk[0]) {
        ...
        //Mapping parameter response from RMS ( to remove underscore symbol at parameter key )
        $params = array(
                    array(
                        "lang_code"     => "en",
                        "name"          => "nbcb",
        ...
        echo "<script>alert('Razer Merchant Sercvices was successfully created!');window.history.back();</script>"; exit;
    } else {
        echo "<script>alert('Razer Merchant Sercvices already set up');window.history.back();</script>"; exit;
    }

}

if( $mode == "getChannel" ) {
    $sspp  = db_query("SELECT processor_id FROM ?:payment_processors WHERE processor = ?s", "Razer Merchant Services Online Payment");
